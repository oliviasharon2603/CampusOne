            ...
            <div className="space-y-12 relative">
              {roadmap.map((node, idx) => {
                ...
                >
                  ...
                  {/* Node Content Card */}
                  <div className={`w-[calc(100%-6rem)] ml-24 md:ml-0 md:w-[45%] bg-white rounded-2xl shadow-sm border p-6 transition-all relative ${
                    ...
                    <div className={`hidden md:block absolute top-1/2 -translate-y-1/2 w-4 h-4 bg-white border-t border-r border-gray-100 transform ${isEven ? '-left-2 rotate-45 border-l-0 border-b-0' : '-right-2 rotate-45 border-t-0 border-r-0 border-l border-b'}`}></div>
                    ...
                    {node.duration && (
                      <div className="flex flex-wrap items-center gap-3 mt-3">
                         <span className="flex items-center text-xs font-bold text-gray-500 bg-gray-100 px-2 py-1 rounded-md">
                           <Clock className="w-3 h-3 mr-1" /> {node.duration}
                         </span>
                      </div>
                    )}

                    <p className={`text-sm mt-3 font-medium flex items-center ${node.status === 'completed' ? 'text-success-600' : 'text-gray-500'}`}>
          ...
          <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full relative z-10 animate-in zoom-in-95 duration-200 overflow-hidden">
            <div className={`p-6 sm:p-8 text-white ${selectedNode.color} relative`}>
              <div className="flex justify-between items-start mb-4">
                <div className="bg-white/20 w-14 h-14 sm:w-16 sm:h-16 rounded-2xl flex items-center justify-center backdrop-blur-md">
                  <Target className="w-7 h-7 sm:w-8 sm:h-8" />
                </div>
                {selectedNode.duration && (
                  <div className="bg-white/20 px-3 py-1.5 rounded-full backdrop-blur-md flex items-center text-sm font-bold border border-white/20">
            ...
            <div className="p-6 sm:p-8 max-h-[60vh] overflow-y-auto">
              {/* Learning Resources */}
              <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">Suggested Resources</h4>
              <div className="flex flex-col gap-3 mb-6">
                {selectedNode.youtubeLink && (
                  <a href={selectedNode.youtubeLink} target="_blank" rel="noopener noreferrer" className="flex items-center justify-between p-3 rounded-xl border border-gray-100 hover:border-red-200 hover:bg-red-50 group transition-colors">
                    <div className="flex items-center">
                      <div className="w-10 h-10 rounded-lg bg-red-100 text-red-600 flex items-center justify-center mr-3">
                        <PlaySquare className="w-5 h-5" />
                      </div>
                      <div>
                        <div className="font-bold text-gray-900 text-sm">YouTube Tutorials</div>
                        <div className="text-xs text-gray-500">Search related videos</div>
                      </div>
                    </div>
                ...
                {selectedNode.website && (
                  <a href={`https://www.google.com/search?q=${encodeURIComponent(selectedNode.website)}`} target="_blank" rel="noopener noreferrer" className="flex items-center justify-between p-3 rounded-xl border border-gray-100 hover:border-blue-200 hover:bg-blue-50 group transition-colors">
                    <div className="flex items-center">
                      <div className="w-10 h-10 rounded-lg bg-blue-100 text-blue-600 flex items-center justify-center mr-3">
                        <BookOpen className="w-5 h-5" />
                      </div>
                      <div>
                        <div className="font-bold text-gray-900 text-sm">Learn on {selectedNode.website}</div>
                        <div className="text-xs text-gray-500">Find courses & articles</div>
                      </div>
                    </div>
              ...
              {selectedNode.hackathon && (
                <div className="mb-8">
                  <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">Apply Your Skills</h4>
