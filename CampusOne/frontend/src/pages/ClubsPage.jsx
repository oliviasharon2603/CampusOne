    ...
    <div className="space-y-8 animate-in fade-in duration-300 relative pb-10">
      ...
      {toast && (
        <div className="fixed top-20 right-8 z-50 animate-in slide-in-from-top fade-in duration-300">
          <div className="bg-white dark:bg-slate-800 border border-success-200 shadow-lg rounded-lg p-4 flex items-start space-x-3 max-w-md">
            <CheckCircle2 className="w-5 h-5 text-success-500 mt-0.5 flex-shrink-0" />
            <p className="text-sm font-medium text-gray-800 dark:text-slate-200 leading-snug">{toast}</p>
            <button onClick={() => setToast(null)} className="text-gray-400 dark:text-slate-300 hover:text-gray-600 dark:text-slate-300 focus:outline-none">
              <X className="w-4 h-4" />
            </button>
          </div>
        </div>
      ...
      {/* Header Section */}
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700/50 p-6 sm:p-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
          <div>
            <h1 className="text-2xl sm:text-3xl font-extrabold text-gray-900 dark:text-white flex items-center">
              <UsersRound className="w-8 h-8 mr-3 text-primary-600 dark:text-primary-400" />
              Clubs & Societies
            </h1>
            <p className="text-gray-500 dark:text-slate-300 mt-2 text-lg max-w-2xl">Find your community, develop new skills, and make lifelong connections outside the classroom.</p>
          </div>
            ...
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm font-bold whitespace-nowrap transition-colors ${
                activeTab === category 
                  ...
                  : 'bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-slate-300 hover:bg-gray-200 dark:bg-slate-700'
              }`}
            >
              {category}
            </button>
      ...
      {filteredClubs.length === 0 ? (
        <div className="text-center py-20 bg-white dark:bg-slate-800 rounded-xl border border-gray-100 dark:border-slate-700/50">
          <Users className="w-16 h-16 text-gray-300 mx-auto mb-4" />
          <h3 className="text-xl font-bold text-gray-900 dark:text-white">No clubs found</h3>
          <p className="text-gray-500 dark:text-slate-300 mt-2">Try adjusting your search or filter criteria.</p>
          <Button variant="outline" className="mt-6" onClick={() => {setSearchQuery(''); setActiveTab('All');}}>Reset Filters</Button>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {filteredClubs.map(club => (
            <div key={club.id} className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700/50 overflow-hidden flex flex-col group hover:shadow-md transition-all duration-300">
              <div 
              ...
              <div className="p-6 flex-1 flex flex-col">
                <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-1 group-hover:text-primary-600 dark:text-primary-400 transition-colors">{club.name}</h3>
                <p className="text-sm font-medium text-gray-500 dark:text-slate-300 mb-4">{club.tagline}</p>
                
                <p className="text-sm text-gray-600 dark:text-slate-300 line-clamp-2 flex-1 mb-6">
                  {club.description}
                </p>
      ...
      {selectedClub && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl max-w-3xl w-full relative z-10 animate-in zoom-in-95 duration-300 max-h-[90vh] flex flex-col overflow-hidden">
            ...
            <div className="h-48 sm:h-64 relative flex-shrink-0">
              ...
              <div className="absolute bottom-6 left-6 right-6">
                <div className="flex flex-wrap gap-2 mb-3">
                  ...
                  <span className="bg-white/20 dark:bg-slate-800/20 text-white backdrop-blur-md px-2.5 py-1 text-xs font-bold uppercase tracking-wider rounded-md flex items-center">
            ...
            <div className="flex-1 overflow-y-auto p-6 sm:p-8 space-y-8">
              ...
              <section>
                ...
                <p className="text-gray-600 dark:text-slate-300 leading-relaxed text-lg">{selectedClub.description}</p>
              </section>
                ...
                <section>
                  ...
                  <div className="space-y-4">
                    {selectedClub.coreTeam.map((member, idx) => (
                      <div key={idx} className="flex items-center gap-4 p-3 rounded-xl bg-gray-50 dark:bg-slate-900 border border-gray-100 dark:border-slate-700/50">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-500 dark:text-slate-300">{member.role} • {member.contact || 'No contact'}</p>
                        </div>
                      </div>
                    ))}
                ...
                <section>
                  ...
                  <div className="space-y-4">
                    {selectedClub.upcomingEvents.map((event, idx) => (
                      <div key={idx} className="p-4 rounded-xl border border-gray-100 dark:border-slate-700/50 hover:border-primary-200 dark:border-primary-700 hover:shadow-sm transition-all group">
                        <h4 className="font-bold text-gray-900 dark:text-white group-hover:text-primary-600 dark:text-primary-400 transition-colors">{event.name}</h4>
                        <div className="mt-3 space-y-2">
                          <p className="text-sm text-gray-500 dark:text-slate-300 flex items-center">
            ...
            <div className="p-6 bg-gray-50 dark:bg-slate-900 border-t border-gray-100 dark:border-slate-700/50 flex gap-4">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl max-w-md w-full relative z-10 animate-in zoom-in-95 duration-200">
            <div className="p-8 text-center">
              <div className="w-16 h-16 rounded-full bg-primary-50 dark:bg-primary-900/30 border-4 border-primary-100 dark:border-primary-800 text-primary-600 dark:text-primary-400 flex items-center justify-center mx-auto mb-4">
                <Users className="w-7 h-7" />
              </div>
              <h3 className="text-2xl font-extrabold text-gray-900 dark:text-white mb-2">Join {activeJoinClub.name}</h3>
              <p className="text-gray-500 dark:text-slate-300 mb-8 leading-relaxed">
              ...
              <div className="space-y-4 mb-8 text-left bg-gray-50 dark:bg-slate-900 p-4 rounded-xl border border-gray-100 dark:border-slate-700/50">
                <div>
                  <label className="block text-xs font-bold text-gray-500 dark:text-slate-300 uppercase tracking-wider mb-2">Why do you want to join? *</label>
                  <textarea 
                    className="w-full bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg p-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 resize-none h-24"
