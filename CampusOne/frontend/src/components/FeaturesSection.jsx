...
const FeaturesSection = () => {
  return (
    <section id="explore" className="py-24 bg-white dark:bg-slate-800 transition-colors duration-300 relative overflow-hidden">
      {/* Background Decor */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center max-w-3xl mx-auto mb-20">
          <h2 className="text-sm font-bold text-indigo-600 dark:text-indigo-400 tracking-widest uppercase mb-3">Everything You Need</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {features.map((feature, index) => (
            <div key={index} className="bg-white dark:bg-slate-800 rounded-3xl p-8 shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 border border-gray-100 dark:border-slate-700/50 group">
              <div className="bg-gray-50 dark:bg-slate-900 group-hover:bg-indigo-50 dark:bg-indigo-900/30 inline-block p-4 rounded-2xl mb-6 transition-colors duration-300">
                {feature.icon}
              </div>
              <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-3">{feature.title}</h3>
              <p className="text-gray-600 dark:text-slate-300 leading-relaxed text-lg">{feature.description}</p>
            </div>
          ))}
        </div>
      </div>
